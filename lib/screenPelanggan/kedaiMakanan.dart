...
class KedaiMakanan extends StatefulWidget {
  static const routeName = "/KedaiMakanan";

  final String? userEmail;

  const KedaiMakanan({Key? key, this.userEmail}) : super(key: key);

  @override
  State<KedaiMakanan> createState() => _ItemListPageState();
}

class _ItemListPageState extends State<KedaiMakanan> {
  ...
  Future<void> _showConfirmationDialog(BuildContext context) async {
    ...
    if (removeItems == true) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (BuildContext context) => MenuScreenPelanggan(),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
        stream:
            FirebaseFirestore.instance.collection('makanan').where('email', isEqualTo: widget.userEmail).snapshots(),
          ...
          actions: [
            TextButton(
              ...
              onPressed: () async {
                ...
                Navigator.pushNamed(context, HomePelanggan.routeName);
              },
            ),
          ],
        );
...
void main() {
  runApp(MaterialApp(
    title: 'Kedai Makanan',
    initialRoute: KedaiMakanan.routeName,
    routes: {
