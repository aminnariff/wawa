...
class UsahawanKomenIklan extends StatefulWidget {
  static const routeName = "/UsahawanKomenIklan";
  const UsahawanKomenIklan({Key? key}) : super(key: key);

  @override
  State<UsahawanKomenIklan> createState() => _ItemListPageState();
}

class _ItemListPageState extends State<UsahawanKomenIklan> {
  FirebaseFirestore firestore = FirebaseFirestore.instance;
  TextEditingController searchController = TextEditingController();
  String searchTerm = '';

  Map<String, int> likedItems = {}; // Map to store the liked item document IDs and their counts
  ...
  Future<void> fetchUsername(String email) async {
    try {
      final snapshot = await firestore.collection('PelangganUsahawan').where('Email', isEqualTo: email).get();
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  IconButton(
                    icon: Icon(
                      Icons.thumb_up,
                      color: likedItems.containsKey(item.id) ? Colors.blue : null,
                    ),
                    onPressed: () {
                      likeItem(item.id);
                    },
              ...
              SizedBox(height: 10),
              StreamBuilder(
                stream: firestore.collection('sukakomen').where('documentId', isEqualTo: item.id).snapshots(),
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          ...
          actions: [
            ElevatedButton(
              onPressed: () {
                saveComment(documentId, comment, currentUserEmail, currentUsername);
  ...
  void saveComment(String documentId, String comment, String currentUserEmail, String currentUserUsername) {
